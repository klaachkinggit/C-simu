...
void Simulation::coraux_save(ofstream &fichier)
{
    ...
    {
        fichier << corail.getPosition().x << " "
                << corail.getPosition().y << " "
                << corail.getAge() << " " << corail.getId() << " "
                << corail.getStatut_cor() << " " << corail.getDir_rot() << " "
                << corail.getStatut_dev() << " "
                << corail.getnb_segs() << " " << endl;
...
void Simulation::scavengers_save(ofstream &fichier)
{
    ...
    {
        fichier << scavenger.getPosition().x << " "
                << scavenger.getPosition().y << " " << scavenger.getAge() << " "
                << scavenger.getRadius() << " "
...
int Simulation::get_nbalgues() const
{
    return algues.size();
}
int Simulation::get_nbcoraux() const
{
    return coraux.size();
}
int Simulation::get_nbscavengers() const
{
...
void Simulation::update_scavenger_age()
{
    ...
    {
        bool bool_defaut(1);
        scavengers[i].set_age(scavengers[i].getAge() + 1, max_life_sca, bool_defaut);
        if (!bool_defaut)
        {
...
bool Simulation::algue_near(S2d &pos, Corail const &cor, Segment const &seg)
{
    ...
    for (size_t i(0); i < algues.size(); i++)
    {
        if (distance(seg.getbase(),
                     algues[i].getPosition()) <=
            seg.getlongueur())
        {
            double angle_i = calcul_angle_2vect(seg.getbase(),
                                                algues[i].getPosition(),
                                                seg.getend());
...
void Simulation::exec_cor_alive_rotate(Corail &cor)
{
    ...
    ostringstream oss;
    adjustSegment(copie_effecteur, angle_near, presence, cor.getDir_rot(), delta_rot,
                  delta_l, oss);
...
{
    string init_corail(" ");
    init_corail += to_string(copie_effecteur.getend().x -
                             cos(copie_effecteur.getangle()) *
                                 (l_repro - l_seg_interne)) +
                   " ";
    init_corail += to_string(copie_effecteur.getend().y -
                             sin(copie_effecteur.getangle()) *
                                 (l_repro - l_seg_interne)) +
                   " ";
    init_corail += "1 ";
                                              ...
                                              int &id_cible_temp, int &compteur)
{
    for (auto &corail : coraux)
    {
        if (!corail.getStatut_cor() && find(ids.begin(), ids.end(),
                                            corail.getId()) == ids.end())
        {
            S2d position_corail_disp = corail.getSegments().back().getend();
            if (compteur == 0 || distance(position_corail_disp,
                                          scavenger.getPosition()) <
                                     distance(position_corail_proche,
                                              scavenger.getPosition()))
            {
...
void Simulation::exec_scav_free(Scavenger &scavenger, std::vector<int> &ids)
{
    ...
    double angle_scav_cible(atan2(position_corail_proche.y -
                                      scavenger.getPosition().y,
                                  position_corail_proche.x -
                                      scavenger.getPosition().x));
    double distance_scav_cible(distance(position_corail_proche,
                                        scavenger.getPosition()));
    bool bool_defaut(1);
...
void Simulation::scav_developpement(Scavenger &scavenger, double angle_scav_cible,
                                    bool bool_defaut)
{
    ...
    else
    {
        scavenger.setForme(scavenger.getPosition(),
                           scavenger.getRadius() + delta_r_sca,
                           bool_defaut);
    }
}
...
void Simulation::exec_scav_mange(Scavenger &scavenger)
{
    ...
    double delta_longueur = (last_seg.getlongueur() < delta_l)
                                ? last_seg.getlongueur()
                                : delta_l;
