...
void MyArea::adjustFrame(int width, int height) {
  ...
  if (new_aspect_ratio >
      default_frame.asp) { // keep yMax and yMin. Adjust xMax and xMin
...
void MyArea::on_draw(const Cairo::RefPtr<Cairo::Context> &cr, int width,
  ...
  if (not empty) // drawing in the Model space
  {
    ...
    orthographic_projection(cr,
                            frame); // set the transformation MODELE to GTKmm
...
MyEvent::MyEvent(Simulation &simul)
    : m_Area(simul), m_Main_Box(Gtk::Orientation::HORIZONTAL, 0),
      ...
      m_Buttons_Box(Gtk::Orientation::VERTICAL, 1), m_Label_General("General"),
      m_Button_exit("exit"), m_Button_open("open"), m_Button_save("save"),
      m_Button_start_stop("start"), m_Button_step("step"),
      ...
      m_Label_maj("mise à jour:        "), data_label_maj("0"),
      m_Algues_Box(Gtk::Orientation::HORIZONTAL, 1),
      m_Label_algues("algues:               "), data_label_algues("0"),
      m_Coraux_Box(Gtk::Orientation::HORIZONTAL, 1),
      m_Label_coraux("coraux:               "), data_label_coraux("0"),
      m_Charognards_Box(Gtk::Orientation::HORIZONTAL, 1),
      m_Label_charognards("charognards:      "), data_label_charognards("0"),
      simu(&simul), disconnect(false), timeout_value(25), // Value for timer
...
void MyEvent::setupButtonsAndSignals() {
  m_Button_exit.signal_clicked().connect(
      sigc::mem_fun(*this, &MyEvent::clicked_exit));
  m_Button_open.signal_clicked().connect(
      sigc::mem_fun(*this, &MyEvent::clicked_open));
  m_Button_save.signal_clicked().connect(
      sigc::mem_fun(*this, &MyEvent::clicked_save));
  m_Button_start_stop.signal_clicked().connect(
      sigc::mem_fun(*this, &MyEvent::clicked_start));
  m_Button_step.signal_clicked().connect(
      sigc::mem_fun(*this, &MyEvent::clicked_step));
  m_Checkbox_algue.signal_toggled().connect(
      sigc::mem_fun(*this, &MyEvent::algues_toggled));

  auto controller = Gtk::EventControllerKey::create();
  controller->signal_key_pressed().connect(
      sigc::mem_fun(*this, &MyEvent::key_pressed), false);
