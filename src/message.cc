...
{
    return string("lifeform at (") + to_string(x) + string(";") + to_string(y) +
           string(") is outside allowed boundaries of [1, max-1]\n");
}

std::string message::lifeform_computed_outside(unsigned id, double x, double y)
{
    return string("lifeform (") + to_string(id) + string(") at (") + to_string(x) +
           string(";") + to_string(y) +
           string(") is outside allowed boundaries of ]0, max[\n");
}

std::string message::segment_length_outside(unsigned id, unsigned l)
{
    return string("lifeform (") + to_string(id) + string(") segment length (") +
           to_string(l) + string(") is outside of [l_repro-l_seg_interne, l_repro[\n");
}
                                       ...
                                       unsigned id2, unsigned index2)
{
    ...
    return string("Lifeform (") + to_string(id1) + string(") segment (") +
           to_string(index1) + string(") collides with Lifeform (") +
           to_string(id2) + string(") segment (") + to_string(index2) + string(")\n");
}
